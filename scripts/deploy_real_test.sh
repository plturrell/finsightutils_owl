#!/bin/bash
# Deploy the system for testing with real documents - No Simulation
...
# No need to modify main.py as we now have a single, consolidated application
echo "Using the consolidated production application"

# Set environment variables for real processing
echo "Setting environment variables for real processing..."
export LOAD_FINANCIAL_MODELS=1
export USE_REAL_PROCESSING=1
export REDIS_URL="$REDIS_URL"
export OWLREADY2_CUDA=1

# Start the main application in the background
echo "Starting main application in $ENVIRONMENT mode on port $APP_PORT..."
echo "Using REAL document processing (no simulation)"
...
echo "   curl http://localhost:$APP_PORT/api/v1/status/TASK_ID -H 'X-API-Key: user-api-key-67890'"
echo ""
echo "4. Get task result using:"
echo "   curl http://localhost:$APP_PORT/api/v1/result/TASK_ID?format=json -H 'X-API-Key: user-api-key-67890'"
echo ""
